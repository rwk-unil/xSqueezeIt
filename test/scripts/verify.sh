...
../../console_app -c -f ${FILENAME} -o ${TMPDIR}/compressed.bin || { echo "Failed to compress ${FILANEM}"; exit 1; }
../../console_app -x -f ${TMPDIR}/compressed.bin -o ${TMPDIR}/uncompressed.bcf || { echo "Failed to uncompress ${FILANEM}"; exit 1; }

command -v bcftools || { echo "Failed to find bcftools, is it installed ?"; exit 1; }
